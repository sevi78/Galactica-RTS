...
from source.gui.Button import ImageButton
...
class InfoPanel(WidgetBase):
    def __init__(self, win, x, y, width, height, isSubWidget, **kwargs):
        ...
        self.text = "Ship: rightclick to move to a planet, or reload the ship. \n\n ctrl and mouse click to navigate \n\n" \
        ...
        self.y = self.pos[1]
        self.border = 10
        self.set_colors(colors.frame_color, (12, 10, 1))
    ...
    def reposition(self):# not working yet in this trashy code
        self.setX(self.parent.building_panel.getX())
        self.setY(self.parent.building_panel.getY())
        self.pos = (self.getX(), self.getY())
    ...
    def draw(self):
        ...
        # draw the frame
        self.rect_frame = pygame.draw.rect(self.win, colors.frame_color, pygame.Rect(self.x, self.y, self.width,
            self.height + 10), 1)
