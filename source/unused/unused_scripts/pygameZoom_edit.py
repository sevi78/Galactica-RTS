    ...
    class Blit(object):
        ...
        def draw(self, surface):
            if hasattr(surface, "name"):
                ...
                scaled = pygame.transform.scale(self.surface.image, (
                int(self.surface.getWidth() * self.outer.zoom), int(self.surface.getHeight() * self.outer.zoom)))
                surface.setImage(scaled)
                surface.setX(self.outer.map_point(self.start[0], self.start[1])[0])
                surface.setY(self.outer.map_point(self.start[0], self.start[1])[1])
            # else:
...
class PygameZoom(object):
    ...
    class GameObject(object):
        def __init__(self, x, y, w, h,outer,win, parent) -> None:
            super().__init__()
        ...
        def draw(self, surface):
            ...
            #pygame.draw.rect(surface, (56,223,87), (new_x, new_y, new_w, new_h), 1)
    ...
    class Blit(object):
        def __init__(self, surface, start,outer, button):
            super().__init__()
            self.surface = surface
            self.start = start
            self.outer = outer
        ...
        def draw(self, surface):
            scaled = pygame.transform.scale(self.surface, (int(self.surface.get_width() * self.outer.zoom), int(self.surface.get_height() * self.outer.zoom)))
            surface.blit(scaled, self.outer.map_point(self.start[0], self.start[1]))
            self.button.setX(self.outer.map_point(self.start[0], self.start[1])[0])
            self.button.setY(self.outer.map_point(self.start[0], self.start[1])[1])
    ...
    def draw_game_object(self, x, y, width, height, win, parent):
        self.shapes.append(self.GameObject(x, y, width, height, self, win, parent))
    ...
    def blit(self, surface, start, button):
        self.shapes.append(self.Blit(surface, start,self, button))
