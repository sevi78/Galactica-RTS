...
class Moveable:
    ...
    def update_position(self):
        ...
        panzoom = utils.Globals.app.pan_zoom_handler
        """
        this sets the new position if object is moved.
        """
        if self.ui_parent != None:
            #self.set_ui_parent_offset(self._x, self._y)
            ...
            #
            # try:
            #     self.x = (self.ui_parent.getX() - self.ui_parent_offset_x)
            #     self.y = (self.ui_parent.getY() - self.ui_parent_offset_y)
            # except AttributeError:
            #     self.x = (self.ui_parent.get_rect().x - self.ui_parent_offset_x)
            #     self.y = (self.ui_parent.get_rect().y - self.ui_parent_offset_y)
        ...
        if not self._hidden and not self._disabled:
            ...
            if self.contains(x, y):
                ...
                if mouseState == MouseState.RELEASE and self.clicked:
                    ...
                    if self.string:
                        source.utils.Globals.app.build(self.string)
                    ...
                    if self.info_text:
                        if self.info_text != "":
                            source.utils.Globals.app.info_panel.text = self.info_text
                            source.utils.Globals.app.info_panel.set_planet_image(self.image, size=(85, 85), align="topright")
            else:
                self.clicked = False
                    ...
                    if self.parent:
                        if hasattr(self.parent, "property"):
                            if self.parent.property == "planet":
                                source.utils.Globals.app.set_selected_planet(self.parent)
...
class ButtonArray(WidgetBase):
    def __init__(self, win, x, y, width, height, shape, **kwargs):
        ...
        self.buttonAttributes = {
            ...
            # 'pressedColour': kwargs.get('pressedColours', colors.frame_color),
